...
import { DeleteVoucherEntry, getVoucher } from "../../../Services/Transaction/TransactionsAPI"
...
const VoucherEntryView: FC<HOCData> = ({ data, currentPage, setSearchTerm, handlePageChange, itemperPage, loading,fitem }) => {
    ...
    const handleEdit = (item:any)=>{
        setItems(item);
        (document.getElementById("editVoucher")as HTMLDialogElement).showModal();
        

    }
    const handleDelete = async (id: any) => {
        try {
            await DeleteVoucherEntry(id);
  ...
  return (
    ...
    <div className="flex justify-center items-center ">
        <div className="overflow-x-auto w-full p-2 m-5 shadow-xl rounded-xl ">
            <ReusableTHeader setSearchTerm={setSearchTerm} name="List of Voucher Entries" createButtonAction={handleCreateButton} />
            {loading ? loadingfall : (
                        ...
                        <tbody key={index}>
                            <tr>
                                ...
                                <td className="flex justify-start gap-3"><button onClick={() => handleEdit(item)} className="btn  btn-xs btn-outline btn-accent" type="button"><CiEdit size={20} /></button>
                                            <button onClick={() => handleDelete(item.v_no)} className="btn btn-xs btn-outline btn-error" type="button"><MdDelete size={20} /></button>
                                        </td>
 ...
 <EditVoucherModal items = {items}/>
</>
  )
}

export default withDataFetching(getVoucher,["firm_name","v_date","receipt_no","stamp","fr_machine"])(VoucherEntryView);
